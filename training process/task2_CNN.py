...
class MyDataset(Dataset): 
    def __init__(self,transform=None,target_transform=None, loader=default_loader):
        ...
        'whiteboard_spray': 8,
        'yellow_block': 9}
        self.imgs = np.load('./data2.npy',allow_pickle=True).tolist()
        with open('./mask.json','r',encoding='utf8')as fp:
            self.lab=json.load(fp)
        self.transform = transform
        self.target_transform = target_transform
        self.loader = loader
    def __getitem__(self, index):
        fn, label,route = self.imgs[index]
        img4=[]
        labs=self.lab['mask_'+route[0]+'_'+route[1]]
        labss=[]
        labss.append((labs[2]-labs[0])/20)
        labss.append((labs[3]-labs[1])/20)
...
if __name__ == "__main__":
    ...
    for epoch in range(48):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            epoch_loss+=loss.item()/50
            if i % 50 == 49:
                print('[%d, %5d] loss: %.3f' %(epoch + 1, i + 1, running_loss / 400))
                running_loss = 0.0
        loss_array.append(epoch_loss)
        torch.save(net.state_dict(),  './MTCNN_task2_'+str(epoch)+'.pth')
        x.append(epoch+1)
    print('Finished Training')
    PATH = './MTCNN_task2.pth'
